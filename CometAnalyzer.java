...
public class CometAnalyzer {
    ...
    public Comet[] cometAnalyzerRun(ImagePlus img_orig, CometConfiguration cometConfiguration) {
        int cometOptions = cometConfiguration.GetCometBooleanFlags();
        ...
        if ((cometOptions & CometConfiguration.COMETFIND_BGCORRECT) != 0) {
            ...
            bSub.rollingBallBackground(ip_gs, radiusRollingBall, false, false, false, false, true);
        }
        // ----------------------------------------
        ...
        // Binarization
        double threshValue = cometConfiguration.GetMinimumAreaThreshold();
        threshValue = (threshValue <= 0.0) ? ip_gs.getMinThreshold() : threshValue;
        setThreshold(ip_gs, (int) threshValue);
        // Morphology
        open_ntimes(ip_gs, 3, 0);
        // Setup particle analyzer
        int paopts = ParticleAnalyzer.SHOW_NONE | ParticleAnalyzer.EXCLUDE_EDGE_PARTICLES
        ...
        int validCount = setValidity(Comets, ip_gs2, cometConfiguration);
    ...
    private void correctBackground(ImageProcessor ip, Comet comet) {
        ...
        comet.cometProfile = getColumnAvg(ip, comet.cometRoi,
                new Rectangle(comet.x, comet.y, comet.width, comet.height));
    ...
    private void setupHead(ImageProcessor ip, Comet comet, int cometOptions) {
        ...
        IJ.log(cometOptions + "");
        if ((cometOptions & CometConfiguration.HEADFIND_AUTO) != 0
                || (cometOptions & CometConfiguration.HEADFIND_BRIGHTEST) != 0) {
            ...
            ImageStatistics brightestStats = ImageStatistics.getStatistics(ipComet, ij.measure.Measurements.ALL_STATS,
                    null);
        ...
        if (((cometOptions & CometConfiguration.HEADFIND_AUTO) != 0 && headValid == false)
                || (cometOptions & CometConfiguration.HEADFIND_PROFILE) != 0) {
    ...
    private int setValidity(Vector<Comet> Comets, ImageProcessor ip, CometConfiguration cometConfiguration) {
        ...
        for (int i = 0; i < Comets.size(); i++) {
            ...
            // Comet area should be larger than
            IJ.log(i + " " + comet.area + " < " + cometConfiguration.GetMinimumAreaThreshold());
            if (comet.area < cometConfiguration.GetMinimumAreaThreshold()) {
                comet.status = Comet.DELETED;
                IJ.log(i + " was found too small of an area");
            }
    ...
    private int getHeadEdge(ImageProcessor ip, Comet comet) {
        ...
        for (int i = 1; i < ddCometProfile.length - 1; i++) {
            if (i < zcross)
                continue;
            if (ddCometProfile[i + 1] <= ddCometProfile[i] && ddCometProfile[i - 1] <= ddCometProfile[i]) {
                ddmax = i;
...
*/
	...
	private class CometParticleAnalyzer extends ParticleAnalyzer {
        ...
        public CometParticleAnalyzer(int options, int measurements, Object dummy, double minSize, double maxSize,
